...
from numpy import full, apply_along_axis, copy, sum, fmax, pi, where, zeros, ones
...
class GlowwormSwarmOptimization(Algorithm):
    ...
    def runIteration(self, task, GS, GS_f, xb, fxb, L, R, rs, **dparams):
        ...
        for i in range(self.n): GS[i] = task.repair(
            GSo[i] + self.s * ((GSo[j[i]] - GSo[i]) / (self.Distance(GSo[j[i]], GSo[i]) + 1e-31)), rnd=self.Rand)
